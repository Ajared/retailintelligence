...
const storeTypeRefinement = (data: {
  store_type?: string;
  store_type_description?: string;
}) => {
  if (
    (data.store_type === 'SHOP' || data.store_type === 'OTHER') &&
    (!data.store_type_description || data.store_type_description.trim() === '')
  ) {
    return false;
  }
  return true;
